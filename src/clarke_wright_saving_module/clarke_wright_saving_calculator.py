...
class ClarkeWrightSavingCalculator:
    ...
    def calculate_distance(
        self, postcodes: dict, pairs: list, circuity: float = 1.2
    ) -> list:
        """ """
        ...
        for pair in pairs:
            ...
            haversine_distance = self._haversine_formula(
                lat_1, lat_2, long_1, long_2, circuity
            )
            distance.append(
                {
    ...
    def calculate_distance_from_origin(
        self,
        postcodes: dict,
        origin_lat: float,
        origin_long: float,
        ...
        for postcode in postcodes:
            ...
            distance = self._haversine_formula(
                lat, origin_lat, long, origin_long, circuity
            )
    ...
    def calculate_saving(
        self, distance: list[dict], distance_from_origin: dict
    ) -> list[tuple]:
        """ """
        savings = list()
        ...
        for data in distance:
            ...
            savings.append((postcode_1, postcode_2, saving))
